...
const UserDetailsCard = ({ about, avatar, contact, email, location, name, role }) => {
    ...
    const handlePracticeClick = (name) => {
        console.log('Clicked', name);
    };
    const requestText = 'Test ';
    //  request.length ? '+' + request.length + ' Student request to join' : '';
    return (
        <Card
            sx={{
                p: 2,
                ...
                <Grid item xs={12}>
                    <Grid container spacing={gridSpacing}>
                        <Grid item xs zeroMinWidth>
                            <Avatar alt={name} size="lg" src="assets/images/profile/user-1.png" />
                        </Grid>
                        <Grid item>
